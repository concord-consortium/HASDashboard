...
then
  ...
  rm -rf "$DISTDIR/*" &&\
  git clone git@github.com:concord-consortium/HASDashboard.git $DISTDIR &&\
  cd $DISTDIR &&\
...
echo "Rebuilding app"
echo  # ENVIRONMENT=$DEPLOY_ENV gulp build-all --production --buildInfo '$SHA built on $DATE' &&\
webpack
...
then
  echo
  echo Deploying to S3 NOT IMPLEMENTED YET.
